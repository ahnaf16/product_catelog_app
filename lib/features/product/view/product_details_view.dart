...
import 'package:product_catelog_app/features/product/view/product_details_skeleton.dart';
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return RefreshIndicator(
      ...
      child: productAsync.when(
        loading: () => const Scaffold(body: ProductDetailsSkeleton()),
        error: (error, _) => _ProductDetailsScaffold(
          body: ContentStateView(
            icon: Icons.cloud_off_rounded,
            title: 'Could not load product',
            message: error.toString(),
            actionLabel: 'Try again',
            onActionTap: detailsCtrl.reload,
          ),
        ),
        data: (product) {
          return Scaffold(
...
class _ProductDetailsScaffold extends StatelessWidget {
  const _ProductDetailsScaffold({required this.body});
